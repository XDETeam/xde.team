...
export const SpectralDesign = () => (
    <Layout>
        ...
        <section>
            ...
            <p>
                Опираемся от ключевой концепции, что объекты многогранны. То есть даже самый
                простой объект содержит столько аспектов, что нет никакой возможности описать
                его сразу с достаточной степенью точности.
            </p>
            ...
            <p>
                ...
                количества контейнеров вокруг объекта создает своеобразную матрёшку, с которой
                очень сложно будет управляться, поскольку возникает обилие комбинаций. Например,
                Nullable&lt;Valid&lt;Request&gt;&gt, Valid&lt;Nullable&lt;Request&gt;&gt;.
            </p>

            <p>
                ADT хоть и выглядят статическо красиво, но очень настораживают, когда речь идёт
                о расширении приложений через плагины и расширения, чтобы реализовать OCP.
            </p>
            ...
            <p>
                В функциональном подходе ещё одним большим вопросом является Dependency Injection.
                И здесь подобные аспекты могут быть как провайдерами данных, так и предоставлять
            ...
            <p>
                ...
                при помощи dependent types. Например, можно сказать, что условный Unsafe&lt;SignIn&gt;,
                у которого пустой Login получает аспект исключения.
            </p>

            <code>{`
                For<SignIn>
                    .Is<Unsafe>()
                    .When(request => request.Login == null)
                    .Then<ValidationException>()
                ;

                For<SignIn>
                    .Is<Unsafe>()
                    .Validate(request => request.Login)
            ...
            <p>
                ...
                компиляторов (Agda, Coq, Arend, etc) в runtime/testtime-реализации могут быть
                свои преимущества. Всё-таки время компиляции существенно влияет на скорость
                разработки и проверка всего кода (даже инкрементальная) не всегда нужна. В
                большинстве случаев мы работаем с конкретным тестом.
            </p>

            <p>
                Попробуем рассмотреть простой HTTP-запрос SignIn. Факт появления запроса на
                некотором роуте приводит к возникновению аспектов SignIn+HTTP+JsonEncoded
                (не хотелось бы хранить SignIn до декодирования, но пока не очень понятно).
                Аспект HTTP можно использовать позже для рендеринга ответа.
            </p>
        </section>
    </Layout>
);
